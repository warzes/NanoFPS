...
bool Example_008::Setup()
{
	...
	// Entities
	{
		vkr::TriMesh mesh = vkr::TriMesh::CreateCube(float3(2, 2, 2), vkr::TriMeshOptions().VertexColors().Normals());
		// 9 total entities. Each uses a descriptor with a uniform buffer allocated from the descriptor pool created above.
		setupEntity(mesh, vkr::GeometryCreateInfo::InterleavedU16().AddColor().AddNormal(), &mInterleavedU16);
		setupEntity(mesh, vkr::GeometryCreateInfo::InterleavedU32().AddColor().AddNormal(), &mInterleavedU32);
		setupEntity(mesh, vkr::GeometryCreateInfo::Interleaved().AddColor().AddNormal(), &mInterleaved);
		setupEntity(mesh, vkr::GeometryCreateInfo::PlanarU16().AddColor().AddNormal(), &mPlanarU16);
		setupEntity(mesh, vkr::GeometryCreateInfo::PlanarU32().AddColor().AddNormal(), &mPlanarU32);
		setupEntity(mesh, vkr::GeometryCreateInfo::Planar().AddColor().AddNormal(), &mPlanar);
		setupEntity(mesh, vkr::GeometryCreateInfo::PositionPlanarU16().AddColor().AddNormal(), &mPositionPlanarU16);
		setupEntity(mesh, vkr::GeometryCreateInfo::PositionPlanarU32().AddColor().AddNormal(), &mPositionPlanarU32);
		setupEntity(mesh, vkr::GeometryCreateInfo::PositionPlanar().AddColor().AddNormal(), &mPositionPlanar);
	}
...
void Example_008::setupEntity(const vkr::TriMesh& mesh, const vkr::GeometryCreateInfo& createInfo, Entity* pEntity)
{
	vkr::Geometry geo;
