...
{
	if (format == Bitmap::FORMAT_UNDEFINED) return ERROR_IMAGE_INVALID_FORMAT;

	// In case of initialization to a preexisting object.
	FreeStbiDataIfNeeded();

	uint32_t minimumRowStride = (width * Bitmap::FormatSize(format));
	if ((rowStride > 0) && (rowStride < minimumRowStride)) return ERROR_BITMAP_FOOTPRINT_MISMATCH;
	...
	if (IsNull(m_data))
	{
		...
		if (n > 0)
		{
			...
			if (m_internalStorage.size() != n) return ERROR_ALLOCATION_FAILED;
...
Result Bitmap::InternalCopy(const Bitmap& obj)
{
	...
	if (m_internalStorage.size() != footprint) return ERROR_ALLOCATION_FAILED;
	m_data = m_internalStorage.data();

	uint64_t srcFootprint = obj.GetFootprintSize();
	if (srcFootprint != static_cast<uint64_t>(footprint)) return ERROR_BITMAP_FOOTPRINT_MISMATCH;
...
Result Bitmap::Create(uint32_t width, uint32_t height, Bitmap::Format format, Bitmap* pBitmap)
{
	...
	if (IsNull(pBitmap)) return ERROR_UNEXPECTED_NULL_ARGUMENT;
...
Bitmap Bitmap::Create(uint32_t width, uint32_t height, Bitmap::Format format, Result* pResult)
{
	...
	if (Failed(ppxres)) bitmap.InternalCtor();
	if (!IsNull(pResult)) *pResult = ppxres;
	return bitmap;
}
...
Result Bitmap::Resize(uint32_t width, uint32_t height)
{
	...
	if (m_internalStorage.empty()) return ERROR_IMAGE_CANNOT_RESIZE_EXTERNAL_STORAGE;
...
{
	if (IsNull(pTargetBitmap))  return ERROR_UNEXPECTED_NULL_ARGUMENT;

	// Format must match
	if (pTargetBitmap->GetFormat() != m_format) return ERROR_IMAGE_INVALID_FORMAT;
	...
	case Bitmap::DATA_TYPE_UNDEFINED: break;
	default: break;
	}
	...
	if (res == 0) return ERROR_IMAGE_RESIZE_FAILED;
...
{
	char* pPixel = nullptr;
	if (!IsNull(m_data) && (x < m_width) && (y < m_height))
	{
...
{
	if (Bitmap::ChannelDataType(m_format) != Bitmap::DATA_TYPE_UINT8) return nullptr;
...
{
	if (Bitmap::ChannelDataType(m_format) != Bitmap::DATA_TYPE_UINT16) return nullptr;
...
{
	if (Bitmap::ChannelDataType(m_format) != Bitmap::DATA_TYPE_UINT32) return nullptr;
...
{
	if (Bitmap::ChannelDataType(m_format) != Bitmap::DATA_TYPE_FLOAT) return nullptr;
...
uint32_t Bitmap::ChannelSize(Bitmap::Format value)
{
	...
	case Bitmap::FORMAT_UNDEFINED: break;
	default: break;
	}
...
static Result IsRadianceFile(const std::filesystem::path& path, bool& isRadiance)
{
	// Open file
	File file;
	if (!file.Open(path.string().c_str())) return ERROR_IMAGE_FILE_LOAD_FAILED;
	// Signature buffer
...
{
	if ((dataSize < kRadianceSigSize) || IsNull(pData)) return ERROR_UNEXPECTED_NULL_ARGUMENT;
...
{
	File file;
	if (!file.Open(path)) return ERROR_IMAGE_FILE_LOAD_FAILED;
	...
	//else 
	{
		...
		stbiResult = stbi_info_from_memory(buffer.data(), static_cast<int>(buffer.size()), pX, pY, pComp);
	}
...
Result Bitmap::GetFileProperties(const std::filesystem::path& path, uint32_t* pWidth, uint32_t* pHeight, Bitmap::Format* pFormat)
{
	...
	if (!IsNull(pWidth)) *pWidth = static_cast<uint32_t>(x);
	if (!IsNull(pHeight)) *pHeight = static_cast<uint32_t>(y);

	// Force to 4 channels to make things easier for the graphics APIs.
	if (!IsNull(pFormat)) *pFormat = isRadiance ? Bitmap::FORMAT_RGBA_FLOAT : Bitmap::FORMAT_RGBA_UINT8;
...
{
	File file;
	if (!file.Open(path)) return nullptr;
	...
	{
		return reinterpret_cast<char*>(stbi_loadf_from_memory(readPtr, static_cast<int>(file.GetLength()), pWidth, pHeight, pChannels, desiredChannels));
	}
	return reinterpret_cast<char*>(stbi_load_from_memory(readPtr, static_cast<int>(file.GetLength()), pWidth, pHeight, pChannels, desiredChannels));
}
...
Result Bitmap::LoadFile(const std::filesystem::path& path, Bitmap* pBitmap)
{
	...
	ppxres = Bitmap::Create(static_cast<uint32_t>(width), static_cast<uint32_t>(height), format, dataPtr, pBitmap);
	if (!pBitmap->IsOk())
	{
...
{
#if defined(_ANDROID)
	Fatal("SaveFilePNG not supported on Android");
	return ERROR_IMAGE_FILE_SAVE_FAILED;
#else
	int res = stbi_write_png(path.string().c_str(), static_cast<int>(pBitmap->GetWidth()), static_cast<int>(pBitmap->GetHeight()), static_cast<int>(pBitmap->GetChannelCount()), pBitmap->GetData(), static_cast<int>(pBitmap->GetRowStride()));
	if (res == 0) return ERROR_IMAGE_FILE_SAVE_FAILED;
	return SUCCESS;
#endif
}
...
{
	if ((dataSize == 0) || IsNull(pData) || IsNull(pBitmap)) return ERROR_UNEXPECTED_NULL_ARGUMENT;
	...
	if (isRadiance)
	{
		pStbData = stbi_loadf_from_memory(static_cast<const stbi_uc*>(pData), static_cast<int>(dataSize), &width, &height, &channels, requiredChannels);
	}
	else
	{
		pStbData = stbi_load_from_memory(static_cast<const stbi_uc*>(pData), static_cast<int>(dataSize), &width, &height, &channels, requiredChannels);
	}

	if (IsNull(pStbData)) return ERROR_IMAGE_FILE_LOAD_FAILED;

	ppxres = Bitmap::Create(static_cast<uint32_t>(width), static_cast<uint32_t>(height), format, static_cast<char*>(pStbData), pBitmap);
	if (!pBitmap->IsOk())
	{
		// Something has gone really wrong if this happens
...
uint32_t CalculatActualLevelCount(uint32_t width, uint32_t height, uint32_t levelCount)
{
	...
	{
		if ((width > 0) && (height > 0)) actualLevelCount += 1;
...
uint64_t CalculateDataSize(uint32_t width, uint32_t height, Bitmap::Format format, uint32_t levelCount)
{
	...
	: mUseStaticPool(useStaticPool)
{
	...
	if (targetData.size() < dataSize) targetData.resize(dataSize);
...
bool Mipmap::IsOk() const
{
	...
	if (format == Bitmap::FORMAT_UNDEFINED) return false;
	...
	if (storageSize < dataSize) return false;
...
{
	Bitmap* ptr = nullptr;
	if (level < GetLevelCount()) ptr = &mMips[level];
	return ptr;
}
...
uint32_t Mipmap::GetWidth([[maybe_unused]] uint32_t level) const
{
	const Bitmap* pMip = GetMip(0);
	return IsNull(pMip) ? 0 : pMip->GetWidth();
}

uint32_t Mipmap::GetHeight([[maybe_unused]] uint32_t level) const
{
	const Bitmap* pMip = GetMip(0);
...
Result Mipmap::LoadFile(const std::filesystem::path& path, uint32_t baseWidth, uint32_t baseHeight, Mipmap* pMipmap, uint32_t levelCount)
{
	...
	for (uint32_t i = 0; i < levelCount; ++i) totalHeight += (baseHeight >> i);

	// Verify that dimensions make sense
	if ((width != baseWidth) || (height < totalHeight)) return ERROR_BITMAP_FOOTPRINT_MISMATCH;

	// Load file
	auto fileBytes = ::LoadFile(path);
	if (!fileBytes.has_value()) return ERROR_IMAGE_FILE_LOAD_FAILED;
	...
	if (IsNull(pStbiData)) return ERROR_BAD_DATA_SOURCE;
...
{
	stbtt_GetFontVMetrics(&mObject->fontInfo, &mObject->ascent, &mObject->descent, &mObject->lineGap);
}

Result Font::CreateFromFile(const std::filesystem::path& path, Font* pFont)
{
	if (IsNull(pFont)) return ERROR_UNEXPECTED_NULL_ARGUMENT;

	if (!std::filesystem::exists(path)) return ERROR_PATH_DOES_NOT_EXIST;

	std::ifstream is(path.c_str(), std::ios::binary);
	if (!is.is_open()) return ERROR_BAD_DATA_SOURCE;
	...
	if (!object) return ERROR_ALLOCATION_FAILED;

	object->fontData.resize(size);
	is.read(reinterpret_cast<char*>(object->fontData.data()), static_cast<std::streamsize>(size));

	int stbres = stbtt_InitFont(&object->fontInfo, object->fontData.data(), 0);
	if (stbres == 0) return Result::ERROR_FONT_PARSE_FAILED;
...
{
	if (IsNull(pFont)) return ERROR_UNEXPECTED_NULL_ARGUMENT;
	if ((size == 0) || IsNull(pData)) return ERROR_BAD_DATA_SOURCE;
...
void Font::GetFontMetrics(float fontSizeInPixels, FontMetrics* pMetrics) const
{
	...
	int descent = 0;
	int lineGap = 0;
	stbtt_GetFontVMetrics(&mObject->fontInfo, &ascent, &descent, &lineGap);

	pMetrics->ascent = static_cast<float>(ascent) * scale;
	pMetrics->descent = static_cast<float>(descent) * scale;
	pMetrics->lineGap = static_cast<float>(lineGap) * scale;
}

void Font::GetGlyphMetrics(float fontSizeInPixels, uint32_t codepoint, float subpixelShiftX, float subpixelShiftY, GlyphMetrics* pMetrics) const
{
	if (IsNull(pMetrics)) return;
	...
	int advanceWidth = 0;
	int leftBearing = 0;
	stbtt_GetCodepointHMetrics(&mObject->fontInfo, static_cast<int>(codepoint), &advanceWidth, &leftBearing);
	pMetrics->advance = static_cast<float>(advanceWidth) * scale;
	pMetrics->leftBearing = static_cast<float>(leftBearing) * scale;
...
void Font::RenderGlyphBitmap(float fontSizeInPixels, uint32_t codepoint, float subpixelShiftX, float subpixelShiftY, uint32_t glyphWidth, uint32_t glyphHeight, uint32_t rowStride, unsigned char* pOutput) const
{
	const float scale = stbtt_ScaleForPixelHeight(&mObject->fontInfo, fontSizeInPixels);
