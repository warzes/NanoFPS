...
public:
	...
	{
		LOG_INFO("Headless mode: skipping initialization of glfw");
...
struct WindowEvents
{
	...
	static void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
	{
		...
		if (!hasKey) {
			LOG_WARN("GLFW key not supported, key=" << key);
		}
	...
	static Result RegisterWindowEvents(GLFWwindow* window, Application* application)
	{
		auto it = sWindows.find(window);
		if (it != sWindows.end()) {
			return ERROR_WINDOW_EVENTS_ALREADY_REGISTERED;
		}
...
public:
	...
	void       FillSurfaceInfo(SurfaceCreateInfo*) const final;
...
Result WindowImplGLFW::Create(const char* title)
{
	...
	if (res != GLFW_TRUE)
	{
		ASSERT_MSG(false, "glfwInit failed");
	...
	if (IsNull(pWindow))
	{
		ASSERT_MSG(false, "glfwCreateWindow failed");
		return ERROR_GLFW_CREATE_WINDOW_FAILED;
	}

	// Register window events
	Result resevent = WindowEvents::RegisterWindowEvents(pWindow, App());
	if (Failed(resevent))
	{
		ASSERT_MSG(false, "RegisterWindowEvents failed");
		return resevent;
	}
	mNative = pWindow;
	return SUCCESS;
...
void WindowImplGLFW::FillSurfaceInfo(SurfaceCreateInfo* pCreateInfo) const
{
#if defined(_WIN32)
